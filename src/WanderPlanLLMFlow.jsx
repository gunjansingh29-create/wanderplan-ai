...
{sc==="crew"&&(<div style={{maxWidth:520}}>
  ...
  {sc==="analytics"&&(<div>
    ...
    {(() => {var totalTripDays=trips.reduce(function(sum,trip){var tripDays=Math.max(0,Number(trip&&trip.days)||0);var fallbackDuration=Math.max(0,Number(trip&&trip.duration_days)||0);return sum+Math.max(tripDays,fallbackDuration);},0);return(<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(min(100%,200px),1fr))",gap:14,marginBottom:24}}>{[{l:"Trips",v:trips.length,c:C.gold},{l:"Destinations",v:bucket.length,c:C.teal},{l:"Total Days",v:totalTripDays,c:C.grn},{l:"Crew",v:crew.length,c:C.sky},{l:"Interests",v:Object.keys(user.interests||{}).length+"/8",c:C.coral}].map(function(s,i){return(<Fade key={i} delay={100+i*50}><div style={{background:C.surface,borderRadius:14,padding:"18px 20px",border:"1px solid "+C.border}}><p style={{fontSize:12,color:C.tx3,marginBottom:6}}>{s.l}</p><p style={{fontWeight:700,fontSize:28,color:s.c}}>{s.v}</p></div></Fade>);})}</div>);})()}
